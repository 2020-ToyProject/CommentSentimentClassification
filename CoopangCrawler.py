...
FILE_NAME = './data/coopang_earphone_normalized_comments.txt'
...
try:
    for pageIdx in range(1, PAGE_MAX_SIZE + 1) :
        ...
        for productId in productIds :
            ...
            # 별점별로 수집
            for currRatingIdx in range(0, 5):
                time.sleep(3)
                action = ActionChains(driver)

                driver.find_element(By.CSS_SELECTOR,
                                    '.sdp-review__article__order__star.js_reviewArticleSearchStarSelectBtn').click()

                currRatingTag = driver.find_elements(By.CSS_SELECTOR,
                                                     '.sdp-review__article__order__star__list__item.js_reviewArticleStarSelectOption')[
                    currRatingIdx]

                try:
                    action.move_to_element(currRatingTag).perform()
                ...
                while True:
                    try:
                        ...
                        for reviewTag in reviewList:
                            ...
                            if 'comment_title' not in review and 'comment_content' not in review:
                                continue
                        ...
                        endCommPageIdx = driver.find_element(By.CSS_SELECTOR,
                            ...
                            'data-end')
                        pageNextIsEnabled = driver.find_element(By.CLASS_NAME,
                                                                'js_reviewArticlePageNextBtn').is_enabled()
